...
class ZaloPlugin(BasePlugin):
    ...
    async def _handle_fetch_members_command(self, request: web.Request) -> web.Response:
        ...
        # Tạo 1 job ảo để theo dõi tiến độ trên giao diện
        job_id = zalo_db.jobs.create(f"Lấy thành viên nhóm {group_id}")
        if job_id:
        ...
        self.logger.info(f"[ZALO] Queued fetch_members command for group {group_id} (Job ID: {job_id})")
        return web.json_response({"status": "ok", "job_id": job_id, "message": f"Đã gửi lệnh lấy thành viên nhóm {group_id}. Chờ JS hooks xử lý..."})
