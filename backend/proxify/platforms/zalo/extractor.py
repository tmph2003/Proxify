...
def inject_hooks_bytes(url: str, body: bytes) -> Optional[bytes]:
    """
    Inject JS hooks into a Zalo JS file using raw bytes to avoid encoding corruption.
    ...
    if "trusted-device-bridge" in url:
        for target, replacement in WASM_HOOKS.items():
            body = body.replace(target.encode('utf-8'), replacement.encode('utf-8'))
    ...
    if b"crypto.subtle.decrypt" in body or b"AES.decrypt" in body:
        body = body + b"\n;\n" + CRYPTO_SUBTLE_HOOK.encode('utf-8')

    # JSON.parse hook (for group Info & message decodeAES outputs)
    if b"JSON.parse(" in body or b"JSON.parse" in body:
        body = body + b"\n;\n" + JSON_PARSE_HOOK.encode('utf-8')
...
def handle_capture_dump(flow: http.HTTPFlow) -> bool:
    ...
    if flow.request.method == "GET" and "api/zalo/commands/pending" in flow.request.path:
        try:
            ...
            logger.error(f"[ZALO] Error polling commands: {e}")
            flow.response = http.Response.make(200, b"[]", {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"})
        return True
        ...
        if content:
            ...
            try:
                ...
                if obj and isinstance(obj, dict):
                    ...
                    if obj.get("_type") == "fetchComplete":
                        group_id = str(obj.get("groupId", ""))
                        if group_id:
                            db.jobs.mark_completed_by_group(group_id)
                            logger.info(f"[ZALO] Marked jobs for group {group_id} as completed.")
...
def modify_zalo_response(flow: http.HTTPFlow) -> None:
    ...
    # Decompress using mitmproxy's decode() to avoid compression bugs, then operate on raw bytes
    flow.response.decode()
    content = flow.response.content
    if not content:
        return

    modified = inject_hooks_bytes(url, content)
    if modified is not None:
        flow.response.content = modified
        # Do NOT modify Content-Type to avoid confusing the browser with mismatched encodings
