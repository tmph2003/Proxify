...
class Dashboard:
    ...
    def _setup_routes(self):
        ...
        self.app.router.add_get("/api/stealth/verify", self._handle_stealth_verify)
        self.app.router.add_get("/api/stealth/stats", self._handle_stealth_stats)
        self.app.router.add_static("/assets", Path(__file__).parent.parent / "assets")
        self.app.router.add_static("/static", Path(__file__).parent / "ui" / "static")
        self.app.router.add_get("/style.css", self._handle_style_css)

        # ── Disable Legacy Plugin Routes for V2 ──
        # In V2, plugins/platforms are registered explicitly in server_v2.py
        # and mounted onto the Dashboard's aiohttp app manually.
        self.plugins = []
        self.ui_tabs = []

        # ── Zalo Extractor Routes (Legacy - to be moved to plugin) ──
        # All Zalo routes have been moved to ZaloPlugin
    ...
    async def _handle_style_css(self, request: web.Request) -> web.Response:
        """Serve the shared CSS."""
        template_path = Path(__file__).parent / "ui" / "templates" / "style.css"
    ...
    async def _handle_index(self, request: web.Request) -> web.Response:
        ...
        # In a real template engine we would inject ui_tabs into HTML here.
        # For now, we can just serve the HTML.
